...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Integer cantTicketsAtendidos = 0; //cantidad de tickets atendidos al final de la simulacion
        ...
        Integer tiempoSimulacion = compiler.calcularTiempoDeSimulacion(24);
        compiler.inicializacionDeSimulacion(cantEmpleados, STO, ITO, TPS);

        while (tiempo < tiempoSimulacion) {

            if (TPLL <= TPS.stream().min(Integer::compare).get()) {
                tiempo = TPLL;
                STLL = STLL + tiempo;
                IA = compiler.calcularIntervaloDeArribo();
                TPLL = tiempo + IA;
                CT = CT + 1;
                cantTicketsAtendidos = cantTicketsAtendidos + 1;
       ...
       /* while (CT != 0) {
        ...
        System.out.println("Cantidad de tickets recibidos en total: " + cantTicketsAtendidos);
        System.out.println("Cantidad de tickets atendidos: " + (cantTicketsAtendidos - CT));
        Float cantTicketsAtendidosPorDia = tiempoSimulacion.floatValue() / cantTicketsAtendidos.floatValue() / 60 * cantidadHoras;
        System.out.println("Cantidad de tickets atendidos por hora: " + cantTicketsAtendidosPorDia);
        System.out.println("Promedio de permanencia en el sistema de ticket " + (STS - STLL) / cantTicketsAtendidos);
